...
char** genThreeScramble(unsigned int scrambleLength)
{
	...
	for (unsigned int i = 0; i < scrambleLength; i++)
	{
		// If the scramble length is less than 15, we know it's a 2x2 scramble, and this function will generate that
		if (scrambleLength < 15) scramble[i][0] = moves[rand() % 3];
		else scramble[i][0] = moves[rand() % 6];
